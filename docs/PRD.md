...
## 1. Problem
NYC commuters often know that a particular line or transfer pattern works better for them, but mainstream routing tools do not let them state preferred lines and independently verify the fastest route that maximizes those choices while still completing a practical trip.
...
- Select one or more preferred subway lines before or after search.
- Compute the fastest route using every preferred line when a valid, sensible route exists.
- When a complete preference match is impossible, return routes using the maximum feasible number of preferred lines.
- Allow the system to fill gaps with walks, transfers, and unselected connector lines.
- Show three ranked routes with familiar transit metrics.

## 4. Core workflow
1. User enters origin and destination.
2. User optionally selects preferred lines.
3. System generates a baseline candidate set and preference-covering candidate sets (BetterMTA orchestration; OTP substrate).
4. System validates timing against schedule and live data.
5. System ranks complete preference matches first, then maximal partial matches, then duration and other tie-breakers.
6. User edits preferred lines and results update.
...
- Selected lines are **preferred lines** for constrained search: maximize coverage; do not require the rider to name every connector (ADR-0023).
- Rider-facing label **S** for the 42 St Shuttle; internal `lineId` remains `GS`.
...
- Brief explanation (including which preferences were omitted when partial).

### Impossible / partial preferences
- Never show a generic “no routes” response when a partial solution exists.
- Return up to three alternatives maximizing preferred-line coverage.
- State which preferred lines are missing.
- When candidate budget is exhausted without adequate preference-covering candidates, surface `insufficient_candidate_coverage` rather than a silent 0-of-N.
...
- Complete preferred-line route ranks above any partial route.
