...
| R37 | A green synthetic/loopback load artifact is mistaken for approved beta capacity, a hosted target result, or a real data-snapshot result | high | medium | Emit commit- and snapshot-bound `SYNTHETIC_LOCAL_PASS_BETA_LOAD_PENDING` evidence with `probeClass: synthetic-local`, `dataSnapshotStatus: synthetic`, `eligibleForGatePass: false`, and `betaCapacityEvidence: false`; serialize in-process runs before output mutation, pass explicit absolute child cwd/scripts, recover verified cwd anchors after hooks, and validate the complete canonical status, fail closed on redirects, cap fixture/request bodies at 1 MiB, enforce all-request finite monotonic latency, exact privacy-safe schemas, anchored real-parent device/inode identity, relative no-follow cleanup, atomically quarantine non-empty final entries before recreating the empty output, final synchronous inventory/type/size/JSON validation, writer-canonical deep equality across probe/result, whole-directory publication, and phase cleanup; retain only bounded probe/result JSON; require a separately owner-approved HTTPS target, real data snapshot, workload, and same-commit artifact before closing `load_p95`. Deterministic in-process races are covered; same-UID external kernel-level races remain a residual limitation of pure Node | Infra + QA + Integration |
...
14. R37 synthetic loopback load artifact versus approved beta capacity and real snapshot evidence
