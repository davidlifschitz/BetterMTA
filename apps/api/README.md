...
Structured logs redact coordinates, address/POI query text, stable query hashes, encrypted geocode PlaceRefs, `providerPlaceId`, and secrets (ADR-0022). Prefer `queryLength` / `proximityProvided` / `proximityGrid` / `selectedLineCount` over raw values.

Helpers for other waves:

- `src/logging/privacy.ts` — redact/coarsen/`PrivacySafe*` builders
